...
def prepare_data():
    # do not modify
    df_train = pd.read_csv(r"/content/gdrive/MyDrive/AI2024final/questioncluster/question classifier (random training).csv")
    df_test = pd.read_csv(r"/content/gdrive/MyDrive/AI2024final/questioncluster/question classifier (random testing).csv")
    '''./data/IMDB_test.csv'''
...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        with torch.no_grad():
            for i, (input_data, label) in enumerate(test_dataloader):
                ...
                if(i < 10):
                  ...
                  print(f"Predicted Output {i + 1}: {torch.argmax(outputs, dim=-1).cpu().numpy()}")
                  print(f"Actual Label {i + 1}: {label.cpu().numpy()}")
        ...
        label_mapping = {'symptoms','treatment',\
              'information','causes',\
              'tests','prevention',\
...
if __name__ == '__main__':
    ...
    label_mapping = {'symptoms':0,'treatment':1,\
              'information':2,'causes':3,\
              'tests':4,'prevention':5,\
              'susceptibility':6,'research':7}
    df_train['label'] = df_train['label'].map(label_mapping)
    df_test['label'] = df_test['label'].map(label_mapping)

    # feel free to add more text preprocessing method
    if preprocessed:
        df_train['question'] = df_train['question'].apply(preprocessing_function)
        df_test['question'] = df_test['question'].apply(preprocessing_function)
